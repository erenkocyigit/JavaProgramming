...
public class ArraysUtility {
    ...
    public static boolean contains(String[] array, String element){
        boolean result = false;
        for (String each : array) {
            if (each.equals(element)){//if any array's element is matching with the given element, it means the given element is
                result = true;
            }
        }
        return result;
    ...
    public static int[] removeElement(int[] array, int index) {
        ...
        for (int i = 0; i < array.length; i++) {
            if (i == index){ // if the index of the array is matching with the given index
                continue;
            }
            result[j++] = array[i];
